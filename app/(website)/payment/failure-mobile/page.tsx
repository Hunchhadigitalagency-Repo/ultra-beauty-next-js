import Link from 'next/link'
import React from 'react'
import { RxCross2 } from 'react-icons/rx'

const Failure: React.FunctionComponent = () => {
    return (
        <div className="flex justify-center items-center border-2 w-full h-56 md:h-96 xl:h-[calc(100vh-160px)] py-20 lg:py-0">
            <div className="flex flex-col gap-4 text-center">
                <div className="h-20 w-20 rounded-full border-2 mx-auto bg-red-600 flex justify-center items-center text-white text-3xl">
                    <RxCross2 />
                </div>
                <h3 className="text-base md:text-lg font-semibold">
                    Payment Failed !
                </h3>
                <p className="text-xs md:text-sm">
                    Opps! Looks like something went wrong. Please try agin with another
                    payment method
                </p>
                <Link href={'/payment'} className="text-xs md:text-sm bg-primary px-5 py-2 mx-auto text-white">
                    Back to payment
                </Link>
            </div>
        </div>
    )
}

export default Failure